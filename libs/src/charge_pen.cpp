...
void LOEF_path::set_is_positive(bool new_value) noexcept {
  this->is_positive_ = new_value;
}
bool LOEF_path::is_positive() const noexcept { return this->is_positive_; }

charge_pen::charge_pen(bool is_positive, vec2d initial_position,
                       millimetre_quantity interval, int max_x, int max_y,
                       dot_per_millimetre_quantity dpmm)
    : is_positive_(is_positive), position_(initial_position),
      interval_(interval), max_x(max_x), max_y(max_y), path(new LOEF_path) {
  path->reserve(5000);
...
step_status charge_pen::step_forward(fixed_charge_map_iterator_ begin,
                                     fixed_charge_map_iterator_ end,
                                     dot_per_millimetre_quantity dpmm) {
  LOEF::electric_field electric_field(0, 0);
  for (auto fixed_charge_itr = begin; fixed_charge_itr != end;
       fixed_charge_itr++) {
    ...
    electric_field +=
        normalize(charge_to_pen).to_dimentionless() *
        (fixed_charge.quantity() / (charge_to_pen.length_square()));
  }
  ...
  if (fuzzy_compare(this->previous_delta_position_,
                    -normalize(electric_field).to_dimentionless() *
                        interval_)) {
  ...
  if (this->is_positive_) {
    ...
    this->position_ +=
        -normalize(electric_field).to_dimentionless() * (interval_);
  }
  this->previous_delta_position_ =
      normalize(electric_field).to_dimentionless() * (interval_);
  ...
  if ((0 <= position_.x() * dpmm && position_.x() * dpmm <= max_x) &&
    ...
    for (auto fixed_iterator = begin; fixed_iterator != end; fixed_iterator++) {
      ...
      if ((fixed_iterator->second.position() - position_).length() <
          (radius::FIXED + 1.0 * millimetre)) {
        if (fixed_iterator->second.pen_arrive(
                this->position_ - fixed_iterator->second.position())) {
